...
export default async function DecisionPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ c?: string; send?: string; skill?: string; params?: string }> }) {
  ...
  const decision = await getDecision(id, ws);
  if (!decision) notFound();
  const [threads, pins, ctx, stats] = await Promise.all([decisionThreads(id, session?.uid ?? null), listPins(id), loadContext(new SkillDb(), ws), workspaceStats(ws)]);
  ...
  const [pane, copy] = await Promise.all([paneContext(messages, ws), askCopy(ws, stats)]);
