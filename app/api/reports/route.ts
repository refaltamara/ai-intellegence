...
export async function POST(req: Request) {
  ...
  const { report } = await createReport({ workspaceId: DEFAULT_WORKSPACE_ID, result, diff: null, source: "ask", title: body.title, decisionId: body.decision_id && /^[0-9a-f-]{36}$/.test(body.decision_id) ? body.decision_id : null });
