...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const [decision, brands, tz] = await Promise.all([
    ...
    sql.query("select id, name from brands where workspace_id = $1", [ws]).then((r) => Object.fromEntries((r as { id: string; name: string }[]).map((b) => [b.id, b.name]))).catch(() => ({})),
    sql.query("select tz from workspaces where id = $1", [ws]).then((r) => (r as { tz: string }[])[0]?.tz ?? "Asia/Jakarta").catch(() => "Asia/Jakarta"),
  ]);
