...
async function buildSystemUncached(workspaceId: string): Promise<string> {
  ...
  const clientLine = profile
    ? (client ? `Every post and comment in this workspace is about ${client.name} (${client.id}); "we" and "us" mean ${client.name}'s team. Read the numbers from their side: what people say about them, how it moves, who drives it. The analyses that answer this are sentiment (the split over time and the spike), comment-themes (what the comments say, in their words), drivers (which posts, accounts and commenters carry it) and seeding (coordinated patterns); start there. The creator analyses (discovery, mercenaries, loyalists, breakout, compare and the rest) are built for brand panels and are not meaningful here; never say the subject is "not in the tracked creator set", and never suggest attaching them to a brand campaign.` : "No subject is set yet for this workspace; treat the tracked name as the subject.")
    : client
